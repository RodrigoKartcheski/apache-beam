...
class Tools:
    ...
    def __init__(self, project=None, path_controller=None, filter_table=False, table=None):
        self.project = project
        self.table = table

        df_controller = pa.read_csv(path_controller)
        df_controller_json = df_controller.to_dict('records')
        
        try:
            if not filter_table:
                self.data_controller = df_controller_json
            else:
                self.data_controller = list(filter(lambda doc: (doc['source_table_name'] == self.table and doc['load_controller'] == 1), df_controller_json))
        except Exception as e:
            logging.error(str(e) + '-- ERRO AO PEGAR TABELA!')

    def get_tables(self):
        tables_list = list(map(lambda key: key['source_table_name'], self.data_controller))

        return tables_list
    
    def get_columns(self, table):
        table_filtred = list(filter(lambda doc: (doc['source_table_name'] == table and doc['load_controller'] == 1), self.data_controller))

        if len(table_filtred) > 0:
            table_filtred = table_filtred[0]
            fields_mapping = table_filtred.get('field_mapping')

            return ', '.join([f"{t[0]}" for t in eval(fields_mapping)])
        
        return False

    def check_table(self, client, table_name, field_partition=None, create_table=False):
        table_filtred = list(filter(lambda doc: (doc['source_table_name'] == table_name and doc['load_controller'] == 1), self.data_controller))

        if len(table_filtred) > 0:
            table_filtred = table_filtred[0]
            table_id = f'{self.project}.{table_filtred.get("dataset_target")}.{table_filtred.get("target_table_name")}'
            
            fields_mapping = eval(table_filtred.get('field_mapping'))
            field_mapping = [(t[0], 'integer') if t[1] == 'int' or t[1] == 'bigint' else t for t in fields_mapping]
            mapping_bigquery = [bigquery.SchemaField(t[0], t[1].upper()) for t in field_mapping]

            table_ref = client.dataset(table_id.split('.')[1], project=self.project).table(table_id.split('.')[2])

            try:
                table = bigquery.Table(table_id, schema=mapping_bigquery)
    ...
    def query_sql(self, table: str, carga_full = False):
        table_filtred = list(filter(lambda doc: (doc['source_table_name'] == table and doc['load_controller'] == 1), self.data_controller))
        ...
        if len(table_filtred) > 0:
            ...
            end_time_str = current_date.strftime('%Y-%m-%d %H:%M:%S')

            columns_select = self.get_columns(table)

            query = f"""
                SELECT {columns_select}
                FROM `{self.project}.{table_filtred["dataset_source"]}.{table_filtred["source_table_name"]}`
            """

            if not carga_full:
                query += f"""
                    WHERE 1 = 1
                    AND CAST(dateload AS TIMESTAMP) >= TIMESTAMP('{start_time_str}')
                    AND CAST(dateload AS TIMESTAMP) < TIMESTAMP('{end_time_str}')
                """
    ...
    def set_type_column(self, column, type_column):
        
        if type_column == "STRING":
            if column is not None:
                return str(column)
            
        elif type_column == "INTEGER":
            if column is not None:
                return int(column)
        ...
        elif type_column == "DATETIME":
            
            if column is not None and not isinstance(column, datetime):
                return datetime.fromtimestamp(column / 1000.0)
            else:
                pass

        elif type_column == "DATE":
            if column is not None and not isinstance(column, datetime):
                return datetime.fromtimestamp(column / 1000.0).date()
            else:
                pass
            ...
            if column is not None:
                ...
                if isinstance(column, datetime):
                    try:
                        column = datetime.strftime(column, '%Y-%m-%d %H:%M:%S %Z')
                    except:
                        column = datetime.strftime(column, '%Y-%m-%d %H:%M:%S')

                column = re.sub(r'(\.\d+|\+\d+:\d+)|(Z)', '', column)
    ...
    def set_schema_json(self, data: list, table: str):
        table_filtred = list(filter(lambda doc: (doc['source_table_name'] == table and doc['load_controller'] == 1), self.data_controller))

        if len(table_filtred) > 0:
            fields_mapping = eval(table_filtred[0].get('field_mapping'))

            doc = {}
            for i in range(len(data)):
                doc[fields_mapping[i][0]] = self.set_type_column(data[i], fields_mapping[i][1])
            
            primary_keys_str = table_filtred[0].get('primary_key')

            doc['primary_keys'] = list(map(lambda x: x.strip(), primary_keys_str.split('|')))
            doc['columns_names'] = {key: key for key in [column.strip() for column in self.get_columns(table).split(',')]}
            doc['table_id'] = f'{self.project}.{table_filtred[0].get("dataset_target")}.{table_filtred[0].get("target_table_name")}'
            doc['source_table'] = table

            field_mapping = [(t[0], 'integer') if t[1] == 'int' or t[1] == 'bigint' else t for t in fields_mapping]
            doc['mapping_bigquery'] = [bigquery.SchemaField(t[0], t[1].upper()) for t in field_mapping]
            doc['field_partition'] = table_filtred[0].get('partition_field')

            return doc
    
        return None
    
    def get_dict_emojis(self):
        dict_emotions = [
    ...
    def extract_emojis(self, data: dict, table: str):
        table_filtred = list(filter(lambda doc: (doc['source_table_name'] == table and doc['load_controller'] == 1 and doc['load_emojis'] == 1), self.data_controller))

        if not table_filtred:
            colum_emojis = table_filtred[0].get('colum_emoji')

            emojis_find = re.findall(r'class="react-emoji".*?\/assets\/upload\/react\/(.*?)\.png', data.get(colum_emojis))

            dict_emojis = self.get_dict_emojis()
            for i, emoji in enumerate(emojis_find):
                if emoji in dict_emojis:
                    emojis_find[i] = dict_emojis[emoji]
            
            data[colum_emojis] = " ".join(emojis_find)

            return data
        
        return data

    def extract_mentions(self, data: dict, table: str):
        table_filtred = list(filter(lambda doc: (doc['source_table_name'] == table and doc['load_controller'] == 1 and doc['load_mentions'] == 1), self.data_controller))

        if not table_filtred:
            colum_mentions = table_filtred[0].get('colum_mentions')

            mentions = re.findall(r'(\s*<a\b[^>]*class=[\"]mentions[^>]*>(.*?)<\/a>\s*)',colum_mentions)
            
            for mention in mentions:
                link, user = mention
                colMentions = re.sub(re.escape(link), f' {user} ', str(colMentions))

            data[colum_mentions] = colMentions
